...
export default function ProductsPage(){
  ...
  const save = async()=>{
    if(!form.category_id || !form.name){ alert('Κατηγορία & Όνομα υποχρεωτικά'); return }
  ...
  const categoriesById = useMemo(()=>Object.fromEntries(cats.map(c=>[String(c.id), c])),[cats])
          ...
          {rows.map(r=>{
            ...
            return (
              <tr key={r.id} style={{borderTop:'1px solid #eee'}}>
                ...
                <td style={{padding:8}}>{cat ? cat.name : ''}</td>
                <td style={{padding:8}}>€ {Number(r.sale_price||0).toFixed(2)}</td>
                <td style={{padding:8}}>{Number(r.stock||0).toFixed(3)}</td>
              </tr>
            )
          })}
        </tbody>
